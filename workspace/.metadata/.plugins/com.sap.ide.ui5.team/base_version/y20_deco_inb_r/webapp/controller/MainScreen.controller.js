sap.ui.define([
  ...
  "sap/ui/core/BusyIndicator"], function (BaseController, formatter, JSONModel, Fragment, BusyIndicator) {
    "use strict";

    return BaseController.extend("y20_deco_inb_r.controller.MainScreen", {
      ...
      onRowSelectionChangeUiTable: function onRowSelectionChangeUiTable(event, selectRow, noFocus) {
        ...
        if (selectedIndex !== -1) {
          ...
          oModel.read("/DocumentItemSet" + setInfo, {
            useBatch: false,
            urlParameters: {
              "$expand": "DocItem_CreatedHUNav,DocItem_NewPackNav,DocItem_ProductNav,DocItem_HUTypeNav"
            },
            success: function success(data, resp) {
              ...
              if (newPackNav.HUType) {
                //const element = _this4.getView().byId("scanPackHUType");
                //element.setSelectedKey(newPackNav.HUType);
              }

              var huTypesNav = data.DocItem_HUTypeNav;
              var jsonModelhutypeNav = new JSONModel(huTypesNav);

              _this4.getView().setModel(jsonModelhutypeNav, "modelHUTypesNav");
              ...
              if (!noFocus) {
                if (!newPackNav.LastHUType) {
                  setTimeout(function () {
                    //return _this4.byId("scanPackHUType").focus(); #ChangeFocus
                    return _this4.byId("scanPackHUType").focus();
                  }, 0);
                } else {
                  setTimeout(function () {
                    //return _this4.byId("scanPackHUType").focus(); #ChangeFocus
                    return _this4.byId("btnPrint").focus();
                  }, 0);
                }
              }
            },
      ...
      onPrintPressed: function onPrintPressed(oEvent) {
        const element = this.getView().byId("scanPackHUType");
        ...
        var selKey = element.getValue();
        this.getView().getModel("modelNewPackNav").getData().HUType = selKey;
        ...
        const createCall = () => {
          ...
          oModel.create("/NewPackingEntCollection", requestBody, {
            success: (successData, response) => {
              ...
              BusyIndicator.hide()
              const cbFunction = successData.Cursor == "RFSCAN" ? this.setFocusOnScanInput : this.setFocusOnPrintButton
      ...
      setCurrentLoggedInUser: function setCurrentLoggedInUser() {
        ...
        xmlHttp.onreadystatechange = function () {
          if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
            ...
            var label = that.getView().getModel("i18n").getResourceBundle().getText("header_user");
            that.byId("headerContentText").setText(label + user);
          }
        };
      ...
      setFocusOnPrintButton: function setFocusOnPrintButton() {
        // #FocusChange
        var input = this.getView().byId("btnPrint");
        setTimeout(function () {
          input.focus();
        }, 0);
      },
