sap.ui.define([
	...
	return Controller.extend("zswl_as_insb.controller.BaseController", {
		whNumber: undefined,
		...
		checkWhNumberAndWorkstation: function  checkWhNumberAndWorkstation() {
			...
			if (!this.whNumber || !this.workstation) {
			  ...
			  location.replace("".concat(appLocation, "?wsMode=INSB&").concat(search.substring(1, search.length)));
			}
		  },
		...
		_getMessagePopover: function () {
			...
			if (!this._oMessagePopover) {
				this._oMessagePopover = sap.ui.xmlfragment(this.getView().getId(),
					"zswl_as_insb.view.MessagePopover", this);
