	...
	return BaseController.extend("y20_obd_qgate.controller.Main", {
		...
		setAutoRefresh: function setAutoRefresh() {
			...
			function checkASBin() {
				...
				MessageToast.show("is AS Bin on Port?" + count);
				count++;
				if(count == 5) {
					clearInterval(autoRefresh);
				}				
		...
		callASIF: function callASIF(method) {
			var that = this;
			...
			var lgnum = "TMPL";
			var workstation = "WC01";
			var sim = "X";
			var param1 = `Lgnum='${lgnum}'`;
			var param2 = `Workstation='${workstation}'`;
			var param3 = `Sim='${sim}'`;
			var url = `/${method}?${param1}&${param2}&${param3}`;
	  ...
	  handleMessage: function(event) {
		...
		onScanHU: function () {
			...
			oModel.read("/ScanHandlingUnitSet(Lgnum='" + lgnum + "',ScanHU='" + huident + "')", {
				...
				success: function (oData, response) {
					...
					that.setTitleTexts(oData.DocumentHeaderEnt.DocNo);
		...
		onSelect: function (event, selectedIndexOld) {
				...
				that.appendTitleText(that.getView().getModel("DlvHeader").getData().DocNo, that.getView().getModel("selectedRow").getData().Itemno);
