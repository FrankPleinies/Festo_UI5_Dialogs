sap.ui.define([
	...
	return BaseController.extend("zswl_as_pick.controller.Main", {
			...
			if (workStation) {
				...
				oModel.read("/LogonCollection" + setInfo, {
					...
					success: (data, response) => {
						...
						that.ASctrl = new ASController(that);
		...
		refreshUIElements: function refreshUIElements() {
			...
			this.hideElement("btn_huconscomplete");
			...
			if (modelTargetHU && modelTargetHU.oData && modelTargetHU.oData.Huident) {
				...
				this.showElement("btn_huconscomplete");
			} else {
		...
		onOpenEmptyBinCheckDialog: function onOpenEmptyBinCheckDialog() {
			...
			if (!popupDialogLow) {
				Fragment.load({
					id: view.getId(),
					name: "zswl_as_pick.view.PopupDialogEmptyBin",
					controller: this
				}).then(function (popupDialogLow) {
					view.addDependent(popupDialogLow);
					popupDialogLow.open();
		...
		onOpenCreateHUDialog: function onOpenCreateHUDialog() {
			...
			if (!popupDialogLow) {
				Fragment.load({
					id: view.getId(),
					name: "zswl_as_pick.view.PopupDialogCreateHU",
					controller: this
				}).then(function (popupDialogLow) {
					view.addDependent(popupDialogLow);
					popupDialogLow.open();
