sap.ui.define([
    ...
    return Controller.extend("y20_deco_inb_r.controller.Base", {
        ...
        handleMessageResponse: function (response, closeCallback) {
            ...
            if (isThereSuccessMessage) {
                ...
                if (response[property].includes("error")) {
                    try {
                        ...
                        let message = ""
                        const msgArr = error.error.innererror.errordetails
                        if (msgArr.length > 1) {
                            message = msgArr.map(msg => msg.message.includes("Service provider") ? "" : msg.message).join("\n");
                        } else {
