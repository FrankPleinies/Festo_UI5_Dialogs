...
sap.ui.define(["sap/ui/core/mvc/Controller", "sap/m/MessageBox", "sap/ui/core/message/Message"], function (Controller, MessageBox, Message) {
  ...
  return Controller.extend("y20_deco_inb_r.controller.Base", {
    handleMessageResponse: function handleMessageResponse(response, closeCallback) {
      ...
      if (isThereSuccessMessage) {
        ...
        if (response[property].includes("error")) {
          try {
            ...
            var message = "";
            var msgArr = error.error.innererror.errordetails;

            if (msgArr.length > 1) {
              message = msgArr.map(function (msg) {
                return msg.message.includes("Service provider") ? "" : msg.message;
              }).join("\n");
            } else {
