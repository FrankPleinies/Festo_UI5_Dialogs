sap.ui.define([
    ...
    return Object.extend("zswl_as_inve.controller.ASController", {
         ...
         openBin: function() {
			...
			oModel.callFunction(method,  { "method": "POST", urlParameters: { Lgnum: lgnum, Workstation: workstation, BinId: asBinId }, 
				success: function (oData, response) {
					...
					view.getView().setModel(oModelASIf, "ASIF");
					var oMessage = new Message({
						message: that.oResourceBundle.getText("Msg_OpenBin"),
						type: MessageType.Warning,
						target: "/Dummy" //,
		...
		closeBin: function(nextHU) {
			...
			oModel.callFunction(method,  { "method": "POST", urlParameters: { Lgnum: lgnum, Workstation: workstation }, 
				success: function (oData, response) {
				  ...
				  view.getView().getModel("HU").setData(null);
				  if(view.getView().getModel("BinContent")) {
					view.getView().getModel("BinContent").setData(null);
				  }
				  if(view.getView().getModel("BinGrid")) {
					view.getView().getModel("BinGrid").setData(null);
				  }
			...
			oModel.callFunction(method,  { "method": "POST", urlParameters: { Lgnum: lgnum, Workstation: workstation }, 
				success: function (oData, response) {
				  ...
				  if(oData.IsReady) {
						...
						view.readCountItems(oData.SelectedBin);
						clearInterval(autoRefresh);
        ...
        handleRunoutMode: function(mode) {
            var view = this.ctrl;
            var that = this;
			var oModel = new sap.ui.model.odata.v2.ODataModel("/sap/opu/odata/sap/ZSWL_EWM_AUTOSTORE_SRV", true);

			var method = "/SetWSRunoutMode";
			var lgnum = this.whNumber;
			var workstation = this.workstation;
			var mode = mode ? "X" : ""; //If button is pressed then send 'X' else ''
			
			oModel.callFunction(method,  { "method": "POST", urlParameters: { Lgnum: lgnum, Workstation: workstation, RunoutMode: mode }, 
				success: function (oData, response) {
					var oModelASIf = new sap.ui.model.json.JSONModel();
					oModelASIf.setData(oData);
					view.getView().setModel(oModelASIf, "ASIF");
					var oMessage = new Message({
						message: that.oResourceBundle.getText("Msg_RunoutMode"),
						type: MessageType.Warning,
						target: "/Dummy" //,
						//processor: view.getModel()
				   });
		...
		//Reread the HU from the model
		reReadHU: function reReadHU() {
            var view = this.ctrl;
		...
		setSelectedCompartment: function setSelectedCompartment(btnText) {
			if(btnText) {
				...
				const found = modelGrid.oData.items.find(element => element.Compartment == btnText);
						if(found) {
							this.selectedCompartment = found.BinID;
						}
			} else {
				this.selectedCompartment = "";
			}
