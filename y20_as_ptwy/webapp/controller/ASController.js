...
], function(Object, MessageStrip, MessageToast, Library, JSONModel, Button, Message, BusyIndicator) {
    "use strict";
	var MessageType = Library.MessageType;
    
    return Object.extend("y20_as_ptwy.controller.ASController", {
        ...
        changeLayout: function(autoRefresh, hutype) {
            var view = this.ctrl;
            var that = this;
			var oModel = new sap.ui.model.odata.v2.ODataModel("/sap/opu/odata/sap/ZSWL_EWM_AUTOSTORE_SRV", true);
			...
			//var lgnum = this.whNumber;
			//var workstation = this.workstation;
			var method = "/ChangeLayout";
			var lgnum = this.whNumber;
			var workstation = this.workstation;
			var asBinId = this.getTopHUIdent();
			
			BusyIndicator.show();
			oModel.callFunction(method,  { "method": "POST", urlParameters: { Lgnum: lgnum, Workstation: workstation, BinId: asBinId, HuType: hutype }, 
				success: function (oData, response) {
					BusyIndicator.hide();
					var oModelASIf = new sap.ui.model.json.JSONModel();
					oModelASIf.setData(oData);
					view.getView().setModel(oModelASIf, "ASIF");
					var oMessage = new Message({
						message: that.oResourceBundle.getText("Msg_ChangeLayout", hutype),
						type: MessageType.Warning,
						target: "/Dummy" //,
						//processor: view.getModel()
				   });
				  ...
				  var popupDialog = view.byId("popupDialogSelection");
				  popupDialog.close();
